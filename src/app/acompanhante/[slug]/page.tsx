...
import { 
  ...
  Clock, DollarSign, Star, Eye, Calendar, Shield, Camera, X, Check,
  Home, Car, Building, Users, Globe, CreditCard, Flag, AlertTriangle, CheckCircle
} from 'lucide-react'
...
function PerfilContent() {
  ...
  // CORREÇÃO: verificação segura do slug
  ...
  // CORREÇÃO: criar cliente dentro do componente
  const supabase = createClient()

  useEffect(() => {
    // CORREÇÃO: só carrega se tiver slug
    if (slug) {
      carregarPerfil()
    } else {
      setCarregando(false)
  ...
  async function carregarPerfil() {
    ...
    try {
      ...
      if (data) {
        ...
        // Incrementar visualização - não crítico
        try {
          await supabase.rpc('incrementar_visualizacao', { p_acompanhante_id: data.id })
        } catch(e) {}
        
        // Carregar avaliações - não crítico
        try {
        ...
        // Carregar disponibilidade - não crítico
        try {
        ...
        // Verificar se está favoritado - não crítico
        try {
  ...
  const compartilhar = () => {
    // CORREÇÃO: verificar se está no browser
  ...
  const enviarDenuncia = async () => {
    if (!motivoDenuncia) {
      setErroDenuncia('Selecione um motivo')
      return
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErroDenuncia('Erro ao enviar. Tente novamente.')
      }
    } catch (e) {
  ...
  const fotos = perfil ? [perfil.foto_capa, ...(perfil.fotos || [])].filter(Boolean) : []
  const diasAnunciada = perfil?.criado_em ? Math.max(0, Math.floor((Date.now() - new Date(perfil.criado_em).getTime()) / (1000 * 60 * 60 * 24))) : 0
  ...
  if (!perfil) {
    return (
      <div style={{minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:'#f5f5f5',padding:'20px'}}>
        <p style={{color:'#666',marginBottom:'16px'}}>Perfil não encontrado</p>
        <Link href="/" style={{color:COR,fontWeight:600}}>Voltar para home</Link>
      </div>
    )
  }
  ...
  return (
    <>
          ...
          <div>
            ...
            <div className="card" style={{marginBottom:'20px'}}>
              <div className="galeria" onClick={() => setMostrarGaleria(true)}>
                {fotos[fotoAtual] && (
                  <img src={fotos[fotoAtual]} alt={perfil.nome} />
                )}
                ...
                <div className="badges">
                  ...
                  {perfil.verificado && (
                    <span className="badge badge-verificado"><Check size={12} /> Verificada</span>
                  )}
                </div>
            ...
            {perfil.sobre_mim?.length > 0 && (
              <div className="card" style={{marginBottom:'20px'}}>
                ...
                <div className="card-body">
                  <div className="tags">
                    {perfil.sobre_mim.map((tag: string, i: number) => (
            ...
            {perfil.servicos?.length > 0 && (
              <div className="card" style={{marginBottom:'20px'}}>
                ...
                <div className="card-body">
                  <div className="tags">
                    {perfil.servicos.map((s: string, i: number) => (
            ...
            {perfil.locais?.length > 0 && (
              <div className="card" style={{marginBottom:'20px'}}>
                ...
                <div className="card-body">
                  <div className="tags">
                    {perfil.locais.map((l: string, i: number) => (
          ...
          <div className="sidebar">
                ...
                <div className="caracteristicas">
                  {perfil.altura && (
                    <div className="caract-item">
                      <span className="caract-label">Altura</span>
                      <span className="caract-valor">{perfil.altura}</span>
                    </div>
                  )}
                  {perfil.peso && (
                    <div className="caract-item">
                      <span className="caract-label">Peso</span>
                      <span className="caract-valor">{perfil.peso}</span>
                    </div>
                  )}
                  {perfil.etnia && (
                    <div className="caract-item">
                      <span className="caract-label">Etnia</span>
                      <span className="caract-valor">{perfil.etnia}</span>
                    </div>
                  )}
                  {perfil.cabelo && (
                    <div className="caract-item">
                      <span className="caract-label">Cabelo</span>
                      <span className="caract-valor">{perfil.cabelo}</span>
                    </div>
                  )}
                  {perfil.olhos && (
                    <div className="caract-item">
                      <span className="caract-label">Olhos</span>
                      <span className="caract-valor">{perfil.olhos}</span>
                    </div>
                  )}
                  {perfil.corpo && (
                    <div className="caract-item">
                      <span className="caract-label">Corpo</span>
                      <span className="caract-valor">{perfil.corpo}</span>
                    </div>
                  )}
                  {perfil.seios && (
                    <div className="caract-item">
                      <span className="caract-label">Seios</span>
                      <span className="caract-valor">{perfil.seios}</span>
                    </div>
                  )}
                  {perfil.pubis && (
                    <div className="caract-item">
                      <span className="caract-label">Púbis</span>
                      <span className="caract-valor">{perfil.pubis}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            {perfil.pagamentos?.length > 0 && (
              <div className="card">
                ...
                <div className="card-body">
                  <div className="tags">
                    {perfil.pagamentos.map((p: string, i: number) => (
            ...
            {perfil.idiomas?.length > 0 && (
              <div className="card">
                ...
                <div className="card-body">
                  <div className="tags">
                    {perfil.idiomas.map((i: string, idx: number) => (
      ...
      {/* BOTÃO DENUNCIAR - No container, no final */}
              ...
              {denunciaEnviada ? (
                <div style={{ textAlign: 'center', padding: '20px 0' }}>
                  ...
                  <p style={{ color: '#666', fontSize: '14px', marginBottom: '20px' }}>
                    Obrigado por ajudar a manter nossa plataforma segura.
                  </p>
                  <button
                    onClick={fecharModalDenuncia}
                    style={{
                      background: COR,
                      color: '#fff',
                      border: 'none',
                ...
                <>
                  <p style={{ color: '#666', fontSize: '14px', marginBottom: '16px' }}>
                    Denunciando: <strong>{perfil.nome}</strong>
                  </p>
                  ...
                  <label style={{ display: 'block', marginBottom: '16px' }}>
                    ...
                    <input
                      type="email"
                      value={emailDenuncia}
                      onChange={e => setEmailDenuncia(e.target.value)}
                  ...
                  {erroDenuncia && (
                    <p style={{ color: '#d32f2f', fontSize: '13px', marginBottom: '12px' }}>{erroDenuncia}</p>
                  )}
                  ...
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: 700,
                      fontSize: '15px',
                      cursor: enviandoDenuncia ? 'wait' : 'pointer',
                      opacity: enviandoDenuncia ? 0.7 : 1
                    }}
                  >
                  ...
                  <p style={{ color: '#999', fontSize: '11px', marginTop: '12px', textAlign: 'center' }}>
                    Denúncias falsas podem resultar em bloqueio.
                  </p>
                </>
              )}
            </div>
...
// CORREÇÃO PRINCIPAL: Wrapper com Suspense para evitar erro de hidratação
