...
// URLs de atendimento que serão tratadas nesta página
...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const canonical = `https://www.vipacompanhante.com/mulheres/${params.estado.toLowerCase()}/${params.cidade}/${params.bairro}/`

  // Verificar se é página de atendimento
  const atendimento = ATENDIMENTOS[params.bairro]
  if (atendimento) {
    return {
      title: `Acompanhantes que ${atendimento.label} em ${cidadeNome} - ${estadoNome}`,
      description: `Encontre acompanhantes que ${atendimento.label.toLowerCase()} em ${cidadeNome}, ${estadoNome}. Perfis verificados com fotos reais.`,
      alternates: { canonical },
      robots: robotsTag,
    }
  }

  // Comportamento original - é bairro
...
export default async function BairroPage({ params, searchParams }: Props) {
  ...
  // Estado inválido -> redireciona pra home
  if (!estadoNome) {
    redirect('/')
  }
  ...
  if (atendimento) {
    ...
    tituloH1 = `Acompanhantes que ${atendimento.label} em ${cidadeNome}`
    textoSEO = `Encontre acompanhantes que ${atendimento.label.toLowerCase()} em ${cidadeNome}, ${estadoNome} na VipAcompanhante. São ${count || 0} perfis verificados. Todos possuem fotos reais e contato direto via WhatsApp.`
    ...
    // SEM PERFIS NO BAIRRO -> REDIRECIONA PRA CIDADE (não dá 404!)
    if (!perfis || perfis.length === 0) {
      redirect(`/${ROTA}/${params.estado}/${params.cidade}/`)
    }

    cidadeNome = perfis[0]?.cidade || cidadeNome
    bairroNome = perfis[0]?.bairro || bairroNome
    tituloH1 = `${TIPO} em ${bairroNome}`
    textoSEO = `Encontre as melhores ${TIPO.toLowerCase()} em ${bairroNome}, ${cidadeNome} - ${estadoNome} na VipAcompanhante. São ${count || 0} perfis verificados no bairro. Todos os perfis possuem fotos reais e contato direto via WhatsApp.`
  }
