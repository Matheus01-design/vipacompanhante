...
const COR = '#9b59b6'
const SEXO = 'trans'
const TIPO = 'Travestis e Transex'
const ROTA = 'trans'
...
export default async function BairroPage({ params, searchParams }: Props) {
  ...
  if (atendimento) {
    ...
    tituloH1 = `Acompanhantes Trans que ${atendimento.label} em ${cidadeNome}`
    textoSEO = `Encontre acompanhantes trans que ${atendimento.label.toLowerCase()} em ${cidadeNome}, ${estadoNome} na VipAcompanhante. São ${count || 0} perfis verificados.`
    ...
    textoSEO = `Encontre ${TIPO.toLowerCase()} em ${bairroNome}, ${cidadeNome} - ${estadoNome} na VipAcompanhante. São ${count || 0} perfis verificados no bairro.`
  }
        ...
        {totalPaginas > 1 && (
          <div style={{display:'flex',gap:'8px',flexWrap:'wrap',justifyContent:'center',marginBottom:'24px'}}>
            {Array.from({length:Math.min(totalPaginas,10)},(_,i)=>i+1).map(p=>(
              <Link key={p} href={`?pagina=${p}`} style={{padding:'8px 14px',border:`1px solid ${p===pagina?COR:'#ddd'}`,borderRadius:'6px',textDecoration:'none',fontSize:'14px',fontWeight:p===pagina?700:400,color:p===pagina?COR:'#555',background:p===pagina?'#f5ebf8':'#fff'}}>
                {p}
              </Link>
            ))}
          </div>
